...
function Hero() {
  return (
    <section className="relative h-[95vh] min-h-[720px] overflow-hidden bg-black">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_50%,transparent_30%,rgba(0,0,0,.75)_100%)]" />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_bottom_left,rgba(132,29,79,.45),transparent_55%)]" />
      <div className="absolute right-0 top-0 h-[60%] w-[40%] bg-[radial-gradient(circle_at_top_right,rgba(30,108,134,.25),transparent_60%)]" />

      {/* Content */}
      <div className="relative mx-auto flex h-full max-w-7xl flex-col justify-end px-6 pb-16 lg:pb-20">
        <div className="max-w-3xl">
          <span className="mb-5 inline-block animate-[fadeIn_1.5s_ease-out_0.2s_forwards] rounded-full border border-white/10 bg-white/5 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.2em] text-white/80 opacity-0 backdrop-blur">
            {hero.badge}
          </span>

          <h1 className="animate-[fadeIn_1.5s_ease-out_0.4s_forwards] text-6xl font-black uppercase leading-[0.9] tracking-tight text-white opacity-0 md:text-7xl lg:text-[88px]">
            {hero.title}
          </h1>
          <p className="animate-[fadeIn_1.5s_ease-out_0.55s_forwards] mt-3 text-lg font-bold uppercase tracking-[0.2em] text-white/70 opacity-0 md:text-xl">
            {hero.subtitle}
          </p>

          <div className="animate-[fadeIn_1.5s_ease-out_0.7s_forwards] mt-6 flex flex-wrap gap-3 opacity-0">
            {[
              `★ ${hero.rating}`,
              hero.platform,
              hero.release,
              ...
              <span
                key={label}
                className="rounded-full bg-white/5 px-4 py-2 text-xs font-semibold text-white/70"
              >
                {label}
              </span>
            ))}
          ...
          <p className="animate-[fadeIn_1.5s_ease-out_0.85s_forwards] mt-4 text-sm font-semibold tracking-wide text-white/50 opacity-0">
            {hero.genres}
          </p>

          <p className="animate-[fadeIn_1.5s_ease-out_1s_forwards] mt-5 max-w-2xl text-base leading-relaxed text-white/70 opacity-0 line-clamp-3">
            {hero.description}
          </p>

          <div className="animate-[fadeIn_1.5s_ease-out_1.15s_forwards] mt-8 flex flex-wrap items-center gap-4 opacity-0">
            <button className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-brand-primary to-brand-cyan px-7 py-3.5 text-sm font-bold uppercase text-white shadow-[0_0_40px_rgba(132,29,79,.35)] transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_0_60px_rgba(132,29,79,.45)]">
